...
namespace NuGet.Commands.Test
{
    public class SourceRepositoryDependencyProviderTests
    {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task GetDependenciesAsync_WhenPackageIsSelectedWithAssetTargetFallback_AndLegacyDependencyResolutionVariableIsSpecified_CorrectDependenciesAreSelected(bool areDependenciesSelected)
        {
            // Arrange
            var testLogger = new TestLogger();
            var cacheContext = new SourceCacheContext();
            ...
            var library = await provider.GetDependenciesAsync(
                new LibraryIdentity("x", NuGetVersion.Parse("1.0.0-beta"), LibraryType.Package),
                inputFramework,
                useLegacyAssetTargetFallbackBehavior: !areDependenciesSelected,
                cacheContext,
                testLogger,
                CancellationToken.None);
            // Assert
